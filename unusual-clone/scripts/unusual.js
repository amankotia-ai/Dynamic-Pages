...
(function() {
  ...
  const fetchFromEdgeFunction = function(apiUrl, userId, referrer, url, retryCount = 0) {
    ...
    const fetchOptions = {
      ...
      headers: {
        ...
        'x-client-info': 'unusual-client/1.1.0',
    ...
    if (!isUsingProxy) {
      ...
      console.log('Unusual: Using proxy server, CORS should be handled');
    }
    
    return fetch(`${apiUrl}/functions/v1/get_content`, fetchOptions)
  ...
  const init = function() {
    ...
    // Define API URL
    // In production, configure this to point to your Supabase project or proxy server
    const isLocalhost = window.location.hostname === 'localhost' || 
                        window.location.hostname === '127.0.0.1' ||
                        window.location.hostname.includes('.webflow.io');
    
    // Prefer deployed proxy server, but fall back to direct Supabase if not available
    // The proxy server handles CORS issues that might arise with direct Supabase connections
    const apiUrl = isLocalhost
      ? 'http://localhost:3000'  // Local development proxy
      : 'https://gckvjovozupvteqivjiv.supabase.co'; // Supabase project URL - replace with your proxy in production
